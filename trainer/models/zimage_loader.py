...
# Based on file location src/diffusers/models/transformers/transformer_z_image.py:
# ".." refers to diffusers.models
...
class ZImageTransformer2DModel(ModelMixin, ConfigMixin, PeftAdapterMixin, FromOriginalModelMixin):
    ...
    def _prepare_sequence(
        ...
        feats_cat[torch.cat(inner_pad_mask)] = pad_token.to(dtype=feats_cat.dtype)
    ...
    def forward(
        ...
        rep_learning_features = None
        target_rep_layer_idx = 7
        ...
        for layer_idx, layer in enumerate(self.layers):
            ...
            # Capture representation learning features from layer 5
            if return_dict and layer_idx == target_rep_layer_idx:
                rep_learning_features = unified
        ...
        x = self.unpatchify(list(unified_out.unbind(dim=0)), x_size, patch_size, f_patch_size, x_pos_offsets)


        # --- Skipped / Fast Forward Path ---
        x_skip = None
        if rep_learning_features is not None:
            # 1. Fast forward through the final 2 layers only
            # Using the same masks and freqs as the main path
            skip_feat = rep_learning_features
            
            # Iterate over the last 2 layers
            for layer in self.layers[-2:]:
                skip_feat = (
                    self._gradient_checkpointing_func(
                        layer, skip_feat, unified_mask, unified_freqs, adaln_input, unified_noise_tensor, t_noisy, t_clean
                    )
                    if torch.is_grad_enabled() and self.gradient_checkpointing
                    else layer(skip_feat, unified_mask, unified_freqs, adaln_input, unified_noise_tensor, t_noisy, t_clean)
                )
            
            # 2. Apply Final Layer to skipped features
            skip_feat = (
                self.all_final_layer[f"{patch_size}-{f_patch_size}"](
                    skip_feat, noise_mask=unified_noise_tensor, c_noisy=t_noisy, c_clean=t_clean
                )
                if omni_mode
                else self.all_final_layer[f"{patch_size}-{f_patch_size}"](skip_feat, c=adaln_input)
            )

            # 3. Unpatchify the skipped features into an image
            x_skip = self.unpatchify(list(skip_feat.unbind(dim=0)), x_size, patch_size, f_patch_size, x_pos_offsets)

        return (x,) if not return_dict else (x, x_skip)
...
def load_zimage_components(args, device=None, weight_dtype=torch.float32):
    ...
    if model_config:
        ...
        transformer = ZImageTransformer2DModel.from_pretrained(
            #args.pretrained_model_name_or_path, 
            "/workspace/shinon/t2i/Diffuser-script/zimage-wds-training-3/checkpoint-3000/",
