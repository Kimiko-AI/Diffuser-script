...
# Changed relative imports to absolute 'diffusers' imports
...
class ZImageTransformer2DModel(ModelMixin, ConfigMixin, PeftAdapterMixin, FromOriginalModelMixin):
    ...
    def __init__(
            ...
            token_drop_ratio=0.0,  # New: token dropping ratio
    ) -> None:
        super().__init__()
        ...
        self.drop_restore_proj = nn.Linear(dim * 2, dim)

        # Init dropping components
    ...
    def forward(
            ...
            patch_size: int = 2,
            f_patch_size: int = 1,
            token_drop_ratio: float = None,  # Added argument
    ):
        """
        ...
        # Determine effective drop ratio
        if token_drop_ratio is None:
            token_drop_ratio = self.token_drop_ratio

        # Disable token dropping if ControlNet is active (shapes won't match)
        ...
        # --- Main transformer layers with Token Dropping ---

        # Define early and late layers for dropping strategy
        num_layers = len(self.layers)
        num_early = 2
        num_late = 2

        # If model is too shallow, disable dropping or adjust
        if num_layers <= (num_early + num_late):
            token_drop_ratio = 0.0

        # Run first blocks (full sequence)
        for layer_idx in range(num_early):
        ...
        # Snapshot for Skip Connection
        unified_skip = unified.clone()

        # Token Dropping
        ids_restore = None
        keep_len = unified.shape[1]

        if token_drop_ratio > 0.0:
            B, L, D = unified.shape
            keep_len = int(L * (1 - token_drop_ratio))
            ...
            # Helper to gather tokens for various dim configs
            def gather_tokens(tensor, indices):
                if tensor is None: return None
                if tensor.ndim == 2:  # (B, L) -> Mask
                    return torch.gather(tensor, 1, indices)
                elif tensor.ndim == 3:  # (B, L, D) -> Feats
                    return torch.gather(tensor, 1, indices.unsqueeze(-1).expand(-1, -1, tensor.size(2)))
                elif tensor.ndim == 4:  # (B, L, D_head, 2) -> RoPE (complex/real view)
                    # RoPE structure depends on implementation, usually (B, L, D, 2) or (B, L, D) complex
                    # Here it comes from RopeEmbedder which returns (B, L, HeadDim) complex/real last dim
                    # The implementation uses complex numbers or last dim=2 in apply_rotary_emb
                    # Assuming standard behavior, expand indices to match trailing dims
                    expand_shape = [-1, -1] + [tensor.size(i) for i in range(2, tensor.ndim)]
                    return torch.gather(tensor, 1,
                                        indices.view(B, keep_len, *([1] * (tensor.ndim - 2))).expand(*expand_shape))
                return tensor

            # Apply drop
            unified = gather_tokens(unified, ids_keep)
            unified_mask = gather_tokens(unified_mask, ids_keep)
            unified_freqs = gather_tokens(unified_freqs, ids_keep)
            unified_noise_tensor = gather_tokens(unified_noise_tensor, ids_keep)

        # Run Middle Blocks (Reduced or Full)
        for layer_idx in range(num_early, num_layers - num_late):
            layer = self.layers[layer_idx]
            unified = (
                self._gradient_checkpointing_func(
                    layer, unified, unified_mask, unified_freqs, adaln_input, unified_noise_tensor, t_noisy, t_clean
            ...
            # Note: controlnet_block_samples logic skipped here as drop_ratio is forced to 0 if controlnet exists

        # Token Restoration & Fusion
        if token_drop_ratio > 0.0:
            B, L_curr, D = unified.shape
            # Append mask tokens
            mask_tokens = self.mask_token.repeat(B, ids_restore.shape[1] - keep_len, 1)
            unified = torch.cat([unified, mask_tokens], dim=1)

            # Restore original order
            unified = torch.gather(unified, 1, ids_restore.unsqueeze(-1).expand(-1, -1, D))

            # Fusion: Concat with skip connection + project
            unified = torch.cat([unified_skip, unified], dim=-1)
            unified = self.drop_restore_proj(unified)

            # Restore original auxiliary tensors for final layers (though unified_mask/freqs technically needed for last layers)
            # We need to rebuild or restore them. Since we saved unified_freqs BEFORE dropping,
            # we need to recover the original versions.
            # However, in the loop above we overwrote unified_freqs/mask.
            # We should have saved them or re-generate them.
            # Ideally, we should clone them before drop, but they are large.
            # Instead, we can re-gather using ids_restore? No, because we dropped from them.
            # Actually, we generated unified_freqs/mask in _build_unified_sequence,
            # so we can just grab them from variables (which are Python references).
            # Wait, `unified = gather(...)` creates a new tensor, but `unified_freqs = gather(...)` overwrites the variable name.
            # To fix this, we should use different variable names for the dropped versions or re-fetch.
            # Correction: We must ensure we have the full `unified_freqs` and `unified_mask` for the final layers.
            # The cleanest way without re-computation is to keep references to the "full" versions before the drop block.
            pass

            # To handle the variable overwrite issue mentioned in comments above,
        # we restart the flow with correct variable scoping:
        ...
        for layer_idx in range(num_early):
            ...
            current_unified = (
                self._gradient_checkpointing_func(
                    layer, current_unified, unified_mask, unified_freqs, adaln_input, unified_noise_tensor, t_noisy,
                    t_clean
                )
                if torch.is_grad_enabled() and self.gradient_checkpointing
                else layer(current_unified, unified_mask, unified_freqs, adaln_input, unified_noise_tensor, t_noisy,
                           t_clean)
            )
            if controlnet_block_samples is not None and layer_idx in controlnet_block_samples:
                current_unified = current_unified + controlnet_block_samples[layer_idx]

        unified_skip = current_unified.clone()

        # Prepare inputs for middle blocks
        mid_unified = current_unified
        mid_mask = unified_mask
        mid_freqs = unified_freqs
        mid_noise = unified_noise_tensor

        ids_restore = None

        if token_drop_ratio > 0.0:
            B, L, D = current_unified.shape
            keep_len = int(L * (1 - token_drop_ratio))
            ...
            def gather_helper(t, idx):
                if t is None: return None
                if t.ndim == 2: return torch.gather(t, 1, idx)
                if t.ndim == 3: return torch.gather(t, 1, idx.unsqueeze(-1).expand(-1, -1, t.size(2)))
                # Handle RoPE (B, L, D_head) complex or real
                expand_shape = [-1, -1] + [t.size(i) for i in range(2, t.ndim)]
                return torch.gather(t, 1, idx.view(B, keep_len, *([1] * (t.ndim - 2))).expand(*expand_shape))

            mid_unified = gather_helper(mid_unified, ids_keep)
            mid_mask = gather_helper(mid_mask, ids_keep)
            mid_freqs = gather_helper(mid_freqs, ids_keep)
            mid_noise = gather_helper(mid_noise, ids_keep)
        ...
        for layer_idx in range(num_early, num_layers - num_late):
            ...
            mid_unified = (
                self._gradient_checkpointing_func(
                    layer, mid_unified, mid_mask, mid_freqs, adaln_input, mid_noise, t_noisy, t_clean
                )
                if torch.is_grad_enabled() and self.gradient_checkpointing
                else layer(mid_unified, mid_mask, mid_freqs, adaln_input, mid_noise, t_noisy, t_clean)
            )

        # 3. Restore and Fuse
        if token_drop_ratio > 0.0:
            B, _, D = mid_unified.shape
            mask_tokens = self.mask_token.repeat(B, ids_restore.shape[1] - keep_len, 1)
            mid_unified = torch.cat([mid_unified, mask_tokens], dim=1)

            # Restore order
            mid_unified = torch.gather(mid_unified, 1, ids_restore.unsqueeze(-1).expand(-1, -1, D))

            # Fusion
            current_unified = torch.cat([unified_skip, mid_unified], dim=-1)
            current_unified = self.drop_restore_proj(current_unified)
        else:
            current_unified = mid_unified

        # 4. Run Final Blocks
        for layer_idx in range(num_layers - num_late, num_layers):
            layer = self.layers[layer_idx]
            current_unified = (
                self._gradient_checkpointing_func(
                    layer, current_unified, unified_mask, unified_freqs, adaln_input, unified_noise_tensor, t_noisy,
