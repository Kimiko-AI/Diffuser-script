...
def apply_rotary_emb(
    ...
    """
    Modified apply_rotary_emb that handles both standard 2D RoPE [S, D]
    and batched/dropped 4D RoPE [B, S, 1, D].
    """
    if use_real:
        cos, sin = freqs_cis  # Tuple of [S, D] or [B, S, 1, D]

        # --- FIX: Only unsqueeze if dimensions are missing (Standard 2D case) ---
        if cos.ndim == 2:
            if sequence_dim == 2:
                cos = cos[None, None, :, :]
                sin = sin[None, None, :, :]
        ...
        # If cos.ndim > 2 (e.g. 4D), we assume the user (you) handled broadcasting
        # dimensions manually in the forward pass, so we skip the unsqueeze.
        # ------------------------------------------------------------------------
        ...
        # Apply similar safety check for non-real path if needed
        if isinstance(freqs_cis, torch.Tensor) and freqs_cis.ndim == 2:
...
class Flux2Transformer2DModel(
    ...
    def __init__(
        ...
        self.drop_restore_proj = nn.Linear(self.inner_dim * 2, self.inner_dim)
    ...
    def forward(
        ...
        if USE_PEFT_BACKEND:
            ...
            if joint_attention_kwargs is not None and joint_attention_kwargs.get("scale", None) is not None:
                logger.warning("Passing `scale` via `joint_attention_kwargs` ineffective without PEFT backend.")
        ...
        # Concat RoPE (SeqLen, Dim) -> Tuple(Cos, Sin)
        concat_rotary_emb = (
            torch.cat([text_rotary_emb[0], image_rotary_emb[0]], dim=0),
            torch.cat([text_rotary_emb[1], image_rotary_emb[1]], dim=0),
        )

        # 4. Double Stream Transformer Blocks (Always Full Sequence)
        ...
        # --- PREPARE FOR SINGLE STREAM ---
        # Concatenate text and image streams
        hidden_states = torch.cat([encoder_hidden_states, hidden_states], dim=1)

        # Save state for Skip Connection Fusion later
        x_skip_fusion = hidden_states

        # Define Block Split: Run dropping on all blocks EXCEPT the last 2
        num_single_layers = len(self.single_transformer_blocks)
        cutoff_idx = num_single_layers - 2

        # 5. Token Dropping Logic
        apply_dropping = token_drop_ratio > 0.0 and cutoff_idx > 0

        if apply_dropping:
            B, N, D = hidden_states.shape
            len_keep = int(N * (1 - token_drop_ratio))

            # A. Generate Random Indices
            noise = torch.rand(B, N, device=hidden_states.device)
            ids_shuffle = torch.argsort(noise, dim=1)
            ids_restore = torch.argsort(ids_shuffle, dim=1)
            ids_keep = ids_shuffle[:, :len_keep]

            # B. Drop Hidden States
            # Expand indices for gather: (B, KeepLen) -> (B, KeepLen, D)
            dim_gather_idx = ids_keep.unsqueeze(-1).repeat(1, 1, D)
            hidden_states = torch.gather(hidden_states, dim=1, index=dim_gather_idx)

            # C. Drop RoPE Embeddings (CRITICAL)
            # Standard RoPE is (S, D), we need (B, S_dropped, D) because every batch drops differently
            rotary_cos, rotary_sin = concat_rotary_emb
            rope_dim = rotary_cos.shape[-1]

            # Expand to batch: (S, D) -> (B, S, D)
            rotary_cos_b = rotary_cos.unsqueeze(0).expand(B, -1, -1)
            rotary_sin_b = rotary_sin.unsqueeze(0).expand(B, -1, -1)

            # Gather RoPE
            rope_gather_idx = ids_keep.unsqueeze(-1).repeat(1, 1, rope_dim)
            rotary_cos_dropped = torch.gather(rotary_cos_b, 1, index=rope_gather_idx).unsqueeze(2)
            rotary_sin_dropped = torch.gather(rotary_sin_b, 1, index=rope_gather_idx).unsqueeze(2)

            current_rotary_emb = (rotary_cos_dropped, rotary_sin_dropped)

            # Select Middle Blocks
            middle_blocks = self.single_transformer_blocks[:cutoff_idx]
            final_blocks = self.single_transformer_blocks[cutoff_idx:]
        else:
            # If no dropping, we treat the lists simply
            middle_blocks = self.single_transformer_blocks  # Run all if no dropping
            final_blocks = []  # No separate final pass needed if no dropping
            current_rotary_emb = concat_rotary_emb

        # 6. Run Middle Blocks (Dropped Tokens)
        for block in middle_blocks:
            if torch.is_grad_enabled() and self.gradient_checkpointing:
                ...
                hidden_states = block(
                    ...
                    image_rotary_emb=current_rotary_emb,
                    joint_attention_kwargs=joint_attention_kwargs,
                )

        # 7. Restore & Fuse (Only if dropping happened)
        if apply_dropping:
            # A. Append Mask Tokens
            B, N_dropped, _ = hidden_states.shape
            mask_tokens = self.mask_token.repeat(B, N - N_dropped, 1)
            hidden_states = torch.cat([hidden_states, mask_tokens], dim=1)

            # B. Restore Sequence Order
            dim_restore_idx = ids_restore.unsqueeze(-1).repeat(1, 1, hidden_states.shape[-1])
            hidden_states = torch.gather(hidden_states, dim=1, index=dim_restore_idx)

            # C. Fusion: Concatenate [Skip + Processed] -> Project
            # matches: x_concat = torch.cat([x_skip_fusion, x_concat], dim=-1)
            hidden_states = torch.cat([x_skip_fusion, hidden_states], dim=-1)
            hidden_states = self.drop_restore_proj(hidden_states)

            # D. Switch back to FULL RoPE for final blocks
            current_rotary_emb = concat_rotary_emb

            # 8. Run Final 2 Blocks (Full Tokens)
        ...
        # 9. Output layers
        # Remove text tokens from concatenated stream before output projection
...
def load_lumina2_components(args, device=None, weight_dtype=torch.float32):
    ...
    if args.pretrained_model_name_or_path:
        noise_scheduler = FlowMatchEulerDiscreteScheduler.from_pretrained(
            ...
            subfolder=get_arg("scheduler_subfolder", "scheduler"),
        )
    else:
    ...
    tokenizer = AutoTokenizer.from_pretrained(
        ...
        subfolder=get_arg("tokenizer_subfolder", "tokenizer")
    )
    text_encoder = AutoModelForCausalLM.from_pretrained(
        args.text_encoder_path or args.pretrained_model_name_or_path,
        subfolder=get_arg("text_encoder_subfolder", "text_encoder"),
    ...
    # VAE subfolder default is None because often loaded from separate repo root
    vae = AutoencoderKLFlux2.from_pretrained(
        args.vae_path or args.pretrained_model_name_or_path,
        subfolder=get_arg("vae_subfolder", None),
        device_map=device_map,
        torch_dtype=torch.float32,  # Usually kept in fp32
        low_cpu_mem_usage=True if device_map else False
    )
    vae.config.shift_factor = 0.62
    vae.config.scaling_factor = 0.
    # Determine Transformer Class
    transformer_class_path = getattr(args, "transformer_class_path", "Flux2.Flux2Transformer2DModel")
    ...
    if model_config:
        ...
        transformer = TransformerClass.from_pretrained(
             args.pretrained_model_name_or_path,
            #"/workspace/shinon/t2i/Diffuser-script/zimage-wds-training-3/checkpoint-8000",
             subfolder=get_arg("transformer_subfolder", "transformer"),
